...
export class DownloadComponent implements OnInit {
  ...
  constructor(private dialog: MatDialog,private toastrService: ToastrService,private fb: FormBuilder,private evaluationsService:EvaluationsService,private dialogRef: MatDialogRef<DownloadComponent>) { }
  ...
  onChangeFileInput(){
    ...
    if(!this.acceptFiles.includes(this.form.controls['file'].value.type)){
      ...
      this.toastrService.error('El archivo adjunto no es permitido, solo se admiten archivos con formato .xls, xlsx')
    }else{
      ...
      this.evaluationsService.downloadFilesByBulkFile(this.formDataAttachment).subscribe(
        (response:any) => {
          if(response.status == 200){
            ...
            this.toastrService.error('No fue posible descargarse las evaluaciones','Error');
          }
        },
        () => {
